...
namespace mp_course::cpu_workloads::phase_4{
    ...
    bool calculate_disparity_map(int window_radius, int min_disparity, int max_disparity, Image& left, Image& right, Image& map, ThreadPool& thread_pool){
        if(left.w == right.w && left.h == right.h){
            mp_course::ScopeTimer exec_timer("mp_course::zncc_c_multi_thread::calculate_disparity_map");
