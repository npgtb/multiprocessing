...
namespace mp::cpu_workloads::phase_2{
    ...
    void add_matrix(const float* m1, const float* m2, const int size){
        ...
        float * summation = nullptr;
        if(m1 && m2){
            summation = static_cast<float*>(malloc(size * sizeof(float)));
            for(int i = 0;  i < size; ++i){
                summation[i] = m1[i] + m2[i];
            }
            Profiler::add_info("CPU result checksum: " + std::to_string(simple_checksum_float_array(summation, size)));
            free(summation);
        }
    }
}
