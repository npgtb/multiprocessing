...
namespace mp::cpu_workloads::phase_4_vectorized{
    ...
    void grayscale_image_sse_128_work_chunk(uint32_t* rgba_pixels, uint8_t* gs_pixels, const int chunk_start, const int chunk_end){
        ...
        for(; i + 4 < chunk_end; i += 4){
            ...
            _mm_storeu_si32(gs_pixels + i, packed8);
        }
    ...
    void grayscale_image_avx_512_work_chunk(uint32_t* rgba_pixels, uint8_t* gs_pixels, const int chunk_start, const int chunk_end){
        ...
        for(; i + 16 < chunk_end; i += 16){
            ...
            _mm_storeu_si128((__m128i*)(gs_pixels + i), packed_8);
        }
        //Process the remainder 
        for (; i < chunk_end; ++i) {
            uint32_t pixel = rgba_pixels[i];
    ...
    #if defined(__SSE4_1__)
        uint8_t* disparity_map_pixels = map.data<uint8_t>();
