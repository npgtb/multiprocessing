...
namespace mp::cpu_workloads::phase_3_vectorized{
    ...
    bool grayscale_image_sse_128(Image& image){
        ...
        for(; i + 4 < memory_size; i += 4){
            ...
            _mm_storeu_si32(gs_pixels.data() + i, packed8);
        }
        ...
        for (; i < memory_size; ++i) {
            ...
            gs_pixels[i] = (red_modifier * red_chanel + green_modifier * green_channel + blue_modifier * blue_channel) >> 10;
        }
        image.set<uint8_t>(std::move(gs_pixels), image.w, image.h, ImageFormat::GRAY);
        return true;
    }
#endif
    ...
    bool grayscale_image_avx_512(Image& image){
        ...
        for(; i + 16 < memory_size; i += 16){
            ...
            _mm_storeu_si128((__m128i*)(gs_pixels.data() + i), packed_8);
        }
        //Process the remainder 
        for (; i < memory_size; ++i) {
            uint32_t pixel = rgba_pixels[i];
    ...
    float calculate_zncc_sse_128(int x, int rx, int y, int radius, float lmean, float rmean, Image& left, Image& right){
        uint8_t* lpixels = left.data<uint8_t>();
        uint8_t* rpixels = right.data<uint8_t>();
        float upper = 0.f;
    ...
    bool calculate_disparity_map(const int window_radius, const int min_disparity, const int max_disparity, const bool left_to_right, Image& left, Image& right, Image& map, std::string scope_tag){
        ...
        if(left.w == right.w && left.h == right.h && left.format == ImageFormat::GRAY && right.format == ImageFormat::GRAY && window_radius > 0){
            ...
            std::vector<uint8_t> disparty_map(left.w * left.h);
            const int disparity_direction = left_to_right ? -1 : 1;
            for(int y = 0; y < left.h; ++y){
                for(int x = 0; x < left.w; ++x){
                    ...
                    disparty_map[y * left.w + x] = zncc_max_disparity;
                }
            }
            map.set(std::move(disparty_map), left.w, left.h, ImageFormat::GRAY);
            return true;
    ...
    uint8_t grayscale_disparity(const uint8_t min_disparity, const uint8_t max_disparity, const uint8_t value){
    ...
    bool cross_check_occulsion_disparity_maps(
        ...
        if(
            ...
            std::vector<uint8_t> pp_pixels(left_disparity.w * left_disparity.h);
            uint8_t * left_pixels = left_disparity.data<uint8_t>();
            uint8_t * right_pixels = right_disparity.data<uint8_t>();
            ...
            pp_disparity.set(std::move(pp_pixels), left_disparity.w, left_disparity.h, ImageFormat::GRAY);
            return true;
