...
namespace mp{
    ...
    cl_int OpenCLRuntime::load_file(
        clw::Device& device, const std::string& path, const std::vector<std::string>& kernel_names,
        ...
        if((error_code = program_load.value()->build_program(device_list, compile_options)) != CL_SUCCESS){
