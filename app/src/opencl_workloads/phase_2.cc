...
namespace mp::gpu_workloads::phase_2{
    ...
    void print_platform_info(clw::Platform& platform){
        ...
        for(auto& platform_info_fetch: platform_info_fetches){
            ...
            if(info_size.ok()){
                std::vector<uint8_t> platform_info_fetch_data(info_size.value());
                if(platform.info(platform_info_fetch_data.data(), info_size.value(), platform_info_fetch.info_id) == CL_SUCCESS){
                    platform_info_fetch.data_handler(platform_info_fetch.data_label, platform_info_fetch_data.data());
                }
            }
        }
    }
    ...
    void print_device_info(clw::Device& device){
        ...
        for(auto& device_info_fetch: device_info_fetches){
            ...
            if(info_size.ok()){
                std::vector<uint8_t> device_info_fetch_data(info_size.value());
                if(device.info(device_info_fetch_data.data(), info_size.value(), device_info_fetch.info_id) == CL_SUCCESS){
                    device_info_fetch.data_handler(device_info_fetch.data_label, device_info_fetch_data.data());
                }
            }
        }
    }
    ...
    cl_int initialize(OpenCLRuntime& runtime){
        ...
        return runtime.load_file(device, program_path, {kernel_name}, compile_options);
    }
    ...
    cl_int add_matrix_pipeline(OpenCLRuntime& runtime, std::vector<float>& m1, std::vector<float>& m2, const int matrix_size){
        ...
        if(runtime.kernels.size() > 0){
            ...
            clw::ErrorOr<std::shared_ptr<clw::Event>> matrix_write1 = runtime.cc_queue->write_buffer(buffers[0], false, 0, matrix_memory_size, m1.data(), {});
            clw::ErrorOr<std::shared_ptr<clw::Event>> matrix_write2 = runtime.cc_queue->write_buffer(buffers[1], false, 0, matrix_memory_size, m2.data(), {});
            if(!matrix_write1.ok()){
            ...
            //Read the result
            clw::ErrorOr<std::shared_ptr<clw::Event>> result_read = runtime.cc_queue->read_buffer(buffers[2], false, 0, matrix_memory_size, output.data(), {addition.value()});
            if(!result_read.ok()){
            ...
            Profiler::add_info("OpenCL result checksum: " + std::to_string(simple_checksum_float_array(output)));
