...
namespace mp::gpu_workloads::phase_5{
    ...
    cl_int pipeline(
        ...
        clw::ErrorOr<std::shared_ptr<clw::Event>> pp_queue = queue_work(runtime, postprocess_kernel, 2, split_work_dimensions, nullptr, {zncc_left.value(), zncc_right.value()}, {{0,left_dis}, {1,right_dis}, {2, post_pros}}, {});
        if(!pp_queue.ok()){
