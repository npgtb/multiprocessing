...
namespace mp::gpu_workloads::phase_7_integral_aps{
    ...
    cl_int initialize(
        ...
        const std::string program_path = "data/kernels/phase_7_integral_aps.cl";
    ...
    cl_int allocate_local_memory(
        ...
        const size_t integ_tile_size = integ_group_width * sizeof(uint64_t);
        ...
        Profiler::add_info("Integral local memory allocation (" + std::to_string(integ_tile_size) + ")");
        ...
        {
            {
                integ_row_kernel, std::vector<std::pair<int, int>>{
                    {7, integ_tile_size}, {8, integ_tile_size}
                }
            },
            {
                integ_column_kernel, std::vector<std::pair<int, int>>{
                    {4, integ_tile_size}, {5, integ_tile_size}
                }
            },
        {
                zncc_kernel, std::vector<std::pair<int,int>>{
    ...
    cl_int pipeline(
        ...
        if(
            ...
            (error_code = allocate_local_memory(runtime, window_radius, min_disparity, max_disparity, group_width, group_height, integ_scan_width)) != CL_SUCCESS
        ){
