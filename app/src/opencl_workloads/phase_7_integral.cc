...
namespace mp::gpu_workloads::phase_7_integral{
    ...
    cl_int initialize(
        ...
        const std::string program_path = "data/kernels/phase_7_integral.cl";
        clw::Device device = prefered_device();
        //Inject constants into the kernel
    ...
    cl_int allocate_local_memory(OpenCLRuntime& runtime, const int window_radius, const int min_disparity, const int max_disparity, const int group_width, const int group_height){
        ...
        {
                zncc_kernel, std::vector<std::pair<int,int>>{
                    {14, tile_left_size}, {15, tile_right_size}
                }
            },
        {
                postprocess_kernel, std::vector<std::pair<int, int>>{
                    {5, tile_left_size}, {6, tile_right_size}
                }
            }
        };
    ...
    cl_int pipeline(
        ...
        size_t gwork_dim_integral_row[1] = {integ_height};
        size_t gwork_dim_integral_column[1] = {integ_width};
        ...
        clw::ErrorOr<std::shared_ptr<clw::Event>> left_integral_row = queue_work(runtime, integral_row, 1, gwork_dim_integral_row, nullptr, {left_grayscale.value()}, {{0, left_gray}, {1, left_ism}, {2, left_isq}}, {{6, 0}});
        clw::ErrorOr<std::shared_ptr<clw::Event>> right_integral_row = queue_work(runtime, integral_row, 1, gwork_dim_integral_row, nullptr, {right_grayscale.value()}, {{0, right_gray}, {1, right_ism}, {2, right_isq}}, {{6, 1}});
        ...
        clw::ErrorOr<std::shared_ptr<clw::Event>> left_integral_column = queue_work(runtime, integral_column, 1, gwork_dim_integral_column, nullptr, {left_integral_row.value()}, {{0, left_ism}, {1, left_isq}}, {});
        clw::ErrorOr<std::shared_ptr<clw::Event>> right_integral_column = queue_work(runtime, integral_column, 1, gwork_dim_integral_column, nullptr, {right_integral_row.value()}, {{0, right_ism}, {1, right_isq}}, {});
