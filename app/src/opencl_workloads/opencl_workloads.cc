...
namespace mp::gpu_workloads{
    ...
    void run_zncc_optimized_integral_pipeline_workload(
        ...
        if(left.load_path(stereo_left) && right.load_path(stereo_right)){
            cl_int error_code = 0;
            OpenCLRuntime runtime;
            namespace phase_7 = mp::gpu_workloads::phase_7_integral;
            if((error_code = phase_7::initialize(runtime, window_radius, min_disparity, max_disparity, threshold_value)) == CL_SUCCESS){
                for(int i = 0; i < sample_count; ++i){
    ...
    void run_zncc_optimized_integral_aps_pipeline_workload(
        ...
        mp::Profiler::segment_start("PHASE 7 - ZNCC_OPENCL_INTEGRAL_APS");
        mp::Image left, right, pp_dmap;
        //Load stereo images
        if(left.load_path(stereo_left) && right.load_path(stereo_right)){
            cl_int error_code = 0;
            OpenCLRuntime runtime;
            namespace phase_7 = mp::gpu_workloads::phase_7_integral_aps;
            if((error_code = phase_7::initialize(runtime, window_radius, min_disparity, max_disparity, threshold_value)) == CL_SUCCESS){
                for(int i = 0; i < sample_count; ++i){
                    if(phase_7::pipeline(runtime, left, right, pp_dmap, downscale_factor, window_radius, min_disparity, max_disparity, threshold_value) == CL_SUCCESS && i == 0){
                        pp_dmap.save("depthmap_opencl_integral_aps.png");
                    }
                }
            }
            else{
                Profiler::add_info("ZNCC_OPENCL_INTEGRAL_APS | Failed to initialize opencl runtime: " + std::to_string(error_code));
            }
        }
    }
    ...
    void run_zncc_optimized_integral_aps_cl_pipeline_workload(
        ...
        mp::Profiler::segment_start("PHASE 7 - ZNCC_OPENCL_INTEGRAL_APS_CL");
        mp::Image left, right, pp_dmap;
        //Load stereo images
        if(left.load_path(stereo_left) && right.load_path(stereo_right)){
            cl_int error_code = 0;
            OpenCLRuntime runtime;
            namespace phase_7 = mp::gpu_workloads::phase_7_integral_aps_cl;
            if((error_code = phase_7::initialize(runtime, window_radius, min_disparity, max_disparity, threshold_value)) == CL_SUCCESS){
                for(int i = 0; i < sample_count; ++i){
                    if(phase_7::pipeline(runtime, left, right, pp_dmap, downscale_factor, window_radius, min_disparity, max_disparity, threshold_value) == CL_SUCCESS && i == 0){
                        pp_dmap.save("depthmap_opencl_integral_aps_cl.png");
                    }
                }
            }
            else{
                Profiler::add_info("ZNCC_OPENCL_INTEGRAL_APS_CL | Failed to initialize opencl runtime: " + std::to_string(error_code));
            }
        }
    }
