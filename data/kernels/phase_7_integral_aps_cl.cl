...
__kernel void calculate_integral_map_rows(
    ...
    for(int map_x_base = 0; map_x_base < map_w; map_x_base += lw){
        ...
        ushort last_thread = (min(lw, map_w - map_x_base) -1);

        ulong prefix_sum = work_group_scan_inclusive_add(value);
        ulong prefix_sqr = work_group_scan_inclusive_add(value_sqr);

        ulong group_sum = work_group_broadcast(prefix_sum, last_thread);
        ulong group_sqr = work_group_broadcast(prefix_sqr, last_thread);

        //Make scan inclusive and add carry values
        if(map_x < map_w){
            sum_map_data[map_row + map_x] = prefix_sum + carry_sum;
            square_map_data[map_row + map_x] = prefix_sqr + carry_sqr;
        }
        //Sync
        barrier(CLK_LOCAL_MEM_FENCE);
...
__kernel void calculate_integral_map_columns(
    ...
    for(int map_y_base = 0; map_y_base < map_h; map_y_base += lw){
        ...
        ushort last_thread = (min(lw, map_h - map_y_base) -1);
