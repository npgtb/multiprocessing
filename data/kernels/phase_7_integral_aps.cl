...
__kernel void calculate_integral_map_rows(
    ...
    const int img_w, const int img_h, const int map_w, 
    ...
    //Calculate map row index
    const int map_y = get_group_id(0);
    const int map_row = map_y * map_w;

    //Calculate image row index
    const int img_y = map_y - RADIUS;
    const int img_row = clamp(img_y, 0, img_h - 1) * img_w; 

    //Calculate image X start
    int img_x_start = right == 1 ? ((-RADIUS) - MAX_D_EXC) : -RADIUS;

    //Carry between local group blocks in the row
    __local ulong carry_sum;
    __local ulong carry_sqr;
    ...
    //Loop the row as a local group
    for(int map_x_base = 0; map_x_base < map_w; map_x_base += lw){
        //Map and image position on the row
        int map_x = map_x_base + lx;
        
        //Load value and square from image
        ulong value = 0;
        ulong value_sqr = 0;
        if(map_x < map_w){
            int img_x = clamp((img_x_start + map_x), 0, img_w - 1);
            value = pixels[img_row + img_x];
            value_sqr = value * value;
        }

        //Place it into local memory and sync
        lsm[lx] = value;
        lsq[lx] = value_sqr;
        barrier(CLK_LOCAL_MEM_FENCE);

        //Upsweep (Go up the tree)
        for(int stride = 1; stride < lw; stride <<= 1){
            //Strategicly calculate the index
            //So we sum together values seperated
            //by the stride (Upsweep tree)
            int li = (lx + 1) * stride * 2 - 1;
            if(li < lw){
                lsm[li] += lsm[li - stride];
                lsq[li] += lsq[li - stride];
            }
            //Sync
            barrier(CLK_LOCAL_MEM_FENCE);
        }

        //Zero the roots (last element)
        if(lx == 0){
            lsm[lw - 1] = 0;
            lsq[lw - 1] = 0;
        }
        ...
        //Downsweep (Come down the tree)
        for(int stride = lw >> 1; stride > 0; stride >>= 1){
            int li = (lx + 1) * stride * 2 - 1;
            if(li < lw){
                int offset_li = li - stride;
                //Sum the parent and the left value
                ulong temp = lsm[offset_li];
                lsm[offset_li] = lsm[li];
                lsm[li] += temp;

                temp = lsq[offset_li];
                lsq[offset_li] = lsq[li];
                lsq[li] += temp;
            }
            //Sync
            barrier(CLK_LOCAL_MEM_FENCE);
        ...
        //Make scan inclusive and add carry values
        if(map_x < map_w){
            sum_map_data[map_row + map_x] = lsm[lx] + value + carry_sum;
            square_map_data[map_row + map_x] = lsq[lx] + value_sqr + carry_sqr;
        }
        //Sync
        barrier(CLK_LOCAL_MEM_FENCE);

        //Update the carry values from the last local thread
        if(lx == (min(lw, map_w - map_x_base) -1)){
            carry_sum += lsm[lx] + value;
            carry_sqr += lsq[lx] + value_sqr;
        }
        //Sync
        barrier(CLK_LOCAL_MEM_FENCE);
    }
...
__kernel void calculate_integral_map_columns(
    ...
    //Get map column index
    const int map_x = get_group_id(0);

    //Carry between local group blocks in the column
    __local ulong carry_sum;
    __local ulong carry_sqr;
    ...
    //Loop the column as a local group
    for(int map_y_base = 0; map_y_base < map_h; map_y_base += lw){
        const int map_y = (map_y_base + lx);
        const int map_row = (map_y * map_w);

        //Load value and square from integrals
        ulong value = 0;
        ulong value_sqr = 0;
        if(map_y < map_h){
            value = sum_map_data[map_row + map_x];
            value_sqr = square_map_data[map_row + map_x];
        }
        ...
        //Update the carry values from the last local thread
        if(lx == (min(lw, map_h - map_y_base) -1)){
            carry_sum += lsm[lx] + value;
            carry_sqr += lsq[lx] + value_sqr;
        }
        //Sync
...
void load_tiles(
    ...
    const size_t lgw, const size_t lgh, const int d_dir, const int dr_pad,
