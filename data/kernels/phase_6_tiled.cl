...
__kernel void calculate_disparity_map(
    ...
    uchar zncc_max_d = 0;
    //Calculate Left mean
    float left_mean = calculate_window_mean(llx_halo, ly_halo, radius, ltile, tlw, win_pix_count);
    for(uchar d = min_d; d <= max_d; ++d){
        int ldrx_halo = lrx_halo + (d * d_dir);
        //Calculate right mean and zncc score
        float right_mean = calculate_window_mean(ldrx_halo, ly_halo, radius, rtile, trw, win_pix_count);
        float zncc = calculate_zncc(llx_halo, ldrx_halo, ly_halo, radius, left_mean, right_mean, ltile, rtile, tlw, trw);
...
__kernel void cross_check_occulsion_disparity_maps(
    ...
    if(abs(disparity_value_l - disparity_value_r) > threshold_value || final_value == 0){
        ...
        final_value = calculate_window_non_zero_middle(lx_halo, ly_halo, radius, ltile, tlw);
    }
